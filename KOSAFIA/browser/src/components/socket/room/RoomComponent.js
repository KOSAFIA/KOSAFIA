...
const RoomComponent = ({ roomKey }) => {
  ...
  useEffect(() => {
    const fetchRoomInfo = async () => {
      try {
        ...
        const response = await axios.get(`${baseUrl}/api/rooms/${roomKey}`); // API URL 동적 변경
    ...
    const checkIfHost = async () => {
      try {
        ...
        const response = await axios.post(
          ...
          `${baseUrl}/api/game/host/${roomKey}`, // API URL 동적 변경
  ...
  const handleLeaveRoom = async () => {
    try {
      ...
      await axios.post(
       // `http://localhost:8080/api/rooms/${roomKey}/leave`,
       `${baseUrl}/api/rooms/${roomKey}/leave`, 
        {},
        {
    ...
    <div className="room-container">
      <div className="room-header">
        ...
        <div className="room-button-group">
          ...
          <button className="room-button-exit" onClick={handleLeaveRoom}>
            나가기
          </button>
        </div>
      </div>
