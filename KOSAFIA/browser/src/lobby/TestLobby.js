...
const TestLobby = () => {
  ...
  const fetchRooms = async () => {
    try {
      ...
      const response = await axios.get(`${baseUrl}/api/rooms/all`);
  ...
  const filterRooms = () => {
    ...
    if (searchKeyword) {
      filtered = filtered.filter((room) =>
        room.roomName.includes(searchKeyword)
      ); // 검색어 필터링
    }
  ...
  const tryJoinRoom = async (roomKey) => {
    try {
     ...
     `${baseUrl}/api/rooms/${roomKey}/join`,
            ...
            <div className="room-item" key={room.roomKey}>
              <div className="room-title-container">
                <div className="room-title">
                  {room.roomKey}. {room.roomName}
                </div>
                {room.isPrivate && <div className="lock-icon">🔒</div>}{" "}
                {/* 비밀방 아이콘 추가 */}
              </div>
              ...
              <button
                ...
                disabled={
                  room.players?.length >= room.maxPlayers || room.isPlaying
                }
              >
                입장
              </button>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="password-section">
              ...
              >
                <div
                  className={`switch ${roomDetails.isPrivate ? "active" : ""}`}
                ></div>
              </div>
